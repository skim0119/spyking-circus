...
def collect_saturation(nb_threads, params, erase=False):
    ...
    # For each thread/process collect data.
    for count, node in enumerate(to_explore):
        spiketimes_file = file_out_suff + '.times-%d.data' % node
        templates_file = file_out_suff + '.channels-%d.data' % node
        amplitudes_file = file_out_suff + '.values-%d.data' % node
        ...
        if os.path.exists(templates_file):
            ...
            for j in local_temp:
                idx = numpy.where(templates == j)[0]
                result['timesteps']['elec_' + str(j)].append(spiketimes[idx])
                result['amplitudes']['elec_' + str(j)].append(amplitudes[idx])

    sys.stderr.flush()
    # TODO: find a programmer comment.
    for key in result['timesteps']:
        result['timesteps'][key] = numpy.concatenate(result['timesteps'][key]).astype(numpy.uint32)
        result['amplitudes'][key] = numpy.concatenate(result['amplitudes'][key]).astype(numpy.float32)
        idx = numpy.argsort(result['timesteps'][key])
        result['timesteps'][key] = result['timesteps'][key][idx]
        result['amplitudes'][key] = result['amplitudes'][key][idx]
...
def get_overlaps(
    ...
    if comm.rank == 0:
        ...
        to_explore = get_tqdm_progressbar(params, to_explore, desc="precompute overlaps")
    ...
    if comm.rank == 0:
        ...
        over_shape = numpy.array([N_tm**2, duration], dtype=numpy.int64)
